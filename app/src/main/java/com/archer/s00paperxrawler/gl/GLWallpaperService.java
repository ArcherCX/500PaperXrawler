...
//Modifications copyright (C) 2018 Xin Chen
public class GLWallpaperService extends WallpaperService {
    ...
    public class GLEngine extends Engine {
        ...
        public void onOffsetsChanged(float xOffset, float yOffset, float xOffsetStep, float yOffsetStep, int xPixelOffset, int yPixelOffset) {
            mGLThread.offsetsChanged(xOffset, yOffset, xOffsetStep, yOffsetStep, xPixelOffset, yPixelOffset);
        }
...
class GLThread extends Thread {
    ...
    GLThread(GLRenderer renderer, EGLConfigChooser chooser,
             EGLContextFactory contextFactory,
             EGLWindowSurfaceFactory surfaceFactory, GLWrapper wrapper,
             int eglContextClientVersion) {
        super();
    ...
    private void guardedRun() throws InterruptedException {
        ...
        try {
            ...
            while (!isDone()) {
                ...
                if (offsetChanged) {
                    mRenderer.onOffsetsChanged(offsets[0], offsets[1], offsets[2], offsets[3], (int) offsets[4], (int) offsets[5]);
                    offsetChanged = false;
                }
                if ((w > 0) && (h > 0)) {
                    /* draw a frame here */
    ...
    private float[] offsets = new float[6];
    ...
    public void offsetsChanged(float xOffset, float yOffset, float xOffsetStep, float yOffsetStep, int xPixelOffset, int yPixelOffset) {
        synchronized (sGLThreadManager) {
            ...
            offsets[0] = xOffset;
            offsets[1] = yOffset;
            offsets[2] = xOffsetStep;
            offsets[3] = yOffsetStep;
            offsets[4] = xPixelOffset;
            offsets[5] = yPixelOffset;
            mRequestRender = true;
            sGLThreadManager.notifyAll();
        }
    }
